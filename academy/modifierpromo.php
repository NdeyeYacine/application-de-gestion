...
if(!empty($_POST['code']) && !empty($_POST['nom']) && !empty($_POST['mois']) && !empty($_POST['ans'])){
...
$modifierpro=fopen('promo1.txt' , 'r');
while(!feof($modifierpro)){
$ligne=fgets($modifierpro);
$col=explode(',' , $ligne); 
...
$col[2]=$mois;
$col[3]=$annee;
$line=$col[0].",".$col[1].",".$col[2].",".$col[3].","."\n";
}
else{
$line=$ligne;
}

$newlign=$newlign.$line;
}
fclose($modifierpro);

$modifierpro=fopen('promo1.txt' , 'w+');
fwrite($modifierpro,$newlign );
fclose($modifierpro);
}
}

$modifierpro=fopen('promo1.txt' , 'a+');
while(!feof($modifierpro)){
$ligne=fgets($modifierpro);
$col=explode(',', $ligne );
